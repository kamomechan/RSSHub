    ...
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                const featuredImageHTML = $('.post__featured-image').first();
                // 获取 '.post__content-main' 的 HTML 内容
                const contentMainHTML = $('.post__content-main').first().html();

                // 使用模板字符串将两者组合起来
                const combinedHTML = `${featuredImageHTML}${contentMainHTML}`;
