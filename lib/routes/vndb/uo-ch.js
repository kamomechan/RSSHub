...
module.exports = async (ctx) => {
    ...
    const { data: response } = await got(`${baseUrl}/r?q=&o=d&s=released&f=04741Ng114owin122wzh_dHans-2wzh_dHant-`);
    ...
    const item = $('tbody tr')
        ...
        .map((item) => {
            ...
            return {
                ...
                description: `[民间汉化] ${a.attr('title')}<br><br>${item.find('td.tc6 a').attr('href')}`,
                // category: item
                //    .find('a[id^=label]')
                //    .toArray()
                //    .map((item) => $(item).text()),
    ...
    ctx.state.data = {
        ...
        title: `VNDB 民间汉化发布`,
        // 源链接
        link: `${baseUrl}/r?q=&o=d&s=released&f=04741Ng114owin122wzh_dHans-2wzh_dHant-`,
        // 源文章
        item,
    };
};
